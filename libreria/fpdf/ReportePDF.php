...
require_once '../config/conexion.php'; // Conexión directa para reportes
require_once '../libreria/fpdf/fpdf.php';

/* =========================================================
   1. OBTENER DATOS OPTIMIZADOS (SQL DIRECTO)
   Usamos SQL para sumar, es mucho más rápido que PHP.
========================================================= */
...
if (isset($_GET['exportar']) && $_GET['exportar'] === 'pdf') {
    // Limpiamos cualquier salida previa para evitar errores
    ob_end_clean(); 
    
    // Clase interna solo para este reporte resumen
    class PDF_Resumen extends FPDF {
        ...
        function tabla($titulo, $header, $data) {
            ...
            $this->SetFont('Arial','',10);
            foreach($data as $row) {
                // Asumiendo que $row tiene claves 'nombre' y 'total' o 'total_vendido'
